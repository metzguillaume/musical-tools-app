...
const GlobalTools = () => {
    ...
    return (
        <>
            ...
            {/* UPDATED: Added max-h-[85vh] and flex properties to correctly handle overflow. */}
                ...
                <div className="w-11/12 max-w-sm bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl p-4 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                    {activeToolData && (
                        <>
                            ...
                            {/* UPDATED: This container will now grow and scroll if its content is too large. */}
            ...
            {/* Main Toolbar */}
            {/* UPDATED: Added md:bottom-5 to give the sidebar a defined height. */}
            <div className={`fixed z-30 bg-slate-900/80 backdrop-blur-sm
                ...
                {tools.map(tool => (
                    <div key={tool.name} className="bg-slate-800 rounded-lg shadow-lg border border-slate-700 flex-shrink-0 md:flex md:flex-col">
                        ...
                        {/* UPDATED: On desktop, if the tool is active, this container will grow and scroll. */}

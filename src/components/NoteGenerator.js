...
import { useTools } from '../context/ToolsContext'; // Import useTools hook

const NoteGenerator = () => {
    const { bpm, addLogEntry } = useTools(); // Get bpm and addLogEntry from context
    ...
    const handleLogProgress = () => {
        ...
        if (remarks !== null) { // Check if user clicked cancel
            const newEntry = {
                game: 'Note Generator',
                bpm: bpm, // Use the global bpm from context
                date: new Date().toLocaleDateString(),
                remarks: remarks || "No remarks." // Handle empty prompt
            };

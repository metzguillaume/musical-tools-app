...
const IntervalEarTrainer = ({ onProgressUpdate }) => {
    ...
    const {
        ...
        diatonicOptions, newKeyNotification, currentKey, playUserAnswer
    } = useIntervalEarTrainer(settings, onProgressUpdate);
    ...
    useEffect(() => {
        if (currentQuestion) setSelectedNote(null);
        if (currentQuestion && !isAnswered && !isReviewing) {
            const delay = (newKeyNotification && settings.rootNoteMode === 'Roving') ? 3000 : 500;
            const timer = setTimeout(() => playQuestionAudio(), delay);
            return () => clearTimeout(timer);
        }
    }, [currentQuestion, isAnswered, isReviewing, playQuestionAudio, newKeyNotification, settings.rootNoteMode]);

    useEffect(() => {
        if (settings.answerMode === 'Note Names' && settings.autoAdvance && selectedNote !== null && !isAnswered) {
            checkAnswer(selectedNote);
        }
    }, [selectedNote, settings.autoAdvance, settings.answerMode, isAnswered, checkAnswer]);
    ...
    const AnswerButtons = () => {
        ...
        if (settings.answerMode === 'Note Names') {
            ...
            const tonicIndex = noteNameOptions.indexOf(currentKey);
            
            // Rotate the array to start with the tonic and proceed chromatically
            const reorderedNotes = tonicIndex !== -1 
                ? [...noteNameOptions.slice(tonicIndex), ...noteNameOptions.slice(0, tonicIndex)]
                : noteNameOptions;
            // --- END OF UPDATE ---

            return (
                <div className="w-full flex flex-col items-center gap-2">
                    <p className="text-sm text-gray-400">The blue button is the given tonic. Select the second note you heard.</p>
                    <div className="grid grid-cols-6 gap-1 w-full">
                        {reorderedNotes.map(note => {
                            const isTonic = note === currentKey;
                            const isSelected = selectedNote === note;
                            return (
                                <button 
                                    key={note} 
                                    onClick={() => !isTonic && setSelectedNote(note)} 
                                    disabled={buttonsDisabled || isTonic} 
                                    className={`py-3 rounded-md font-semibold disabled:opacity-80 
                                        ${isTonic ? 'bg-blue-600 text-white cursor-default' : 
                                        isSelected ? 'bg-indigo-600 text-white' : 'bg-teal-600 hover:bg-teal-500'}`}
                                >
                                    {note.replace('b','♭').replace('#','♯')}
                                </button>
                            );
                        })}
                    </div>
                    {!settings.autoAdvance && (
                        <div className="flex gap-2 mt-2">
                            <button onClick={() => setSelectedNote(null)} disabled={buttonsDisabled} className="py-2 px-4 rounded-md bg-gray-600 hover:bg-gray-500">Clear</button>
                            <button onClick={() => checkAnswer(selectedNote)} disabled={buttonsDisabled || !selectedNote} className="py-2 px-4 rounded-md bg-blue-600 hover:bg-blue-500 disabled:bg-gray-500">Submit</button>
                        </div>
                    )}
                </div>
            );
    ...
    const renderCorrectAnswerForReview = (item) => {
        ...
        switch(item.answerMode) {
            case 'Note Names': return item.question.answer.noteName;
    ...
    const footerContent = isReviewing ? (
        <div className="flex items-center gap-2">
            ...
            <div className="flex-grow flex flex-col gap-2">
                ...
                {!itemToDisplay.wasCorrect && (
                    <button onClick={() => playUserAnswer(itemToDisplay)} className="text-sm py-1 px-2 rounded-md bg-orange-600 hover:bg-orange-500">Replay Your Answer</button>
                )}
            </div>
        ...
        <div className="flex items-center justify-center flex-wrap gap-2">
            {feedback.type === 'incorrect' && (
                <>
                    <button onClick={() => playQuestionAudio(currentQuestion)} className="py-2 px-4 rounded-md bg-sky-600 hover:bg-sky-500">Replay Question</button>
                    <button onClick={() => playUserAnswer({ question: currentQuestion, userAnswer: history[history.length-1].userAnswer, answerMode: settings.answerMode })} className="py-2 px-4 rounded-md bg-orange-600 hover:bg-orange-500">Replay Your Answer</button>
                </>
            )}
            {(!settings.autoAdvance || feedback.type === 'incorrect') && (
                 <button onClick={generateNewQuestion} className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-lg">Next Question</button>
            )}
        </div>
    ) : null;
